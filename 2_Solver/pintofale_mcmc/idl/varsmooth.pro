function varsmooth,func,scale,xfunc=xfunc,weight=weight,type=type,$
	steps=steps,gauss=gauss,_extra=e
;+
;function	varsmooth
...
;	steps=steps,/gauss,missing=missing,/fwhm,betap=betap,angle=angle)
;
;parameters
...
if n_elements(func) eq 0 then begin
  ...
  print,'       type=type,steps=steps,/gauss,missing=missing,/fwhm,$'
  print,'       betap=betap,angle=angle)'
