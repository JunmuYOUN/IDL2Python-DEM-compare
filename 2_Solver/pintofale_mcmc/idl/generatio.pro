...
for i=0L,nrc-1L do begin	;{for each flux value
  ...
  for j=0L,nss-1L do begin	;{for each use of flux in some ratio
    ...
    if ok eq 'ok' then begin	;(correct format
      rr=c1+'F'+strtrim(i,2)
      if keyword_set(idxrat) then begin	;(not the first ratio
...
for i=0L,nrat-1L do begin
  cc='rx['+strtrim(i,2)+']=('+ratnum[i]+')/('+ratden[i]+')'
...
if arg_present(fxerr) then begin		;(propagate errors?
  ...
  if not keyword_set(dfx_mul) then dfx_mul=0.05
  if not keyword_set(dfx_add) then dfx_add=0.05
  drx_dfx=fltarr(nrat,nfx) & fxp=fltarr(nfx) & fxm=fltarr(nfx)
  for i=0L,nfx-1L do jnk=execute('FXP['+strtrim(i,2)+']=(1.+dfx_mul[0])*fx['+$
	strtrim(i,2)+']+dfx_add[0]')
  for i=0L,nfx-1L do jnk=execute('FXM['+strtrim(i,2)+']=(1.-dfx_mul[0])*fx['+$
	strtrim(i,2)+']+dfx_add[0]')
  for i=0L,nrat-1L do begin	;{for each ratio in question
    for j=0L,nfx-1L do begin	;{for each flux value in the list
      for k=0L,nfx-1L do jnk=execute('F'+strtrim(k,2)+'=fx['+strtrim(k,2)+']')
      jnk=execute('F'+strtrim(j,2)+'=FXP['+strtrim(j,2)+']')
      jnk=execute('RXP=('+ratnum[i]+')/('+ratden[i]+')')
