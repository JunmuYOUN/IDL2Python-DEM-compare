...
if nz eq 0 then nh_ne(*)=1.	;ignore if Z is not given, because
				;this is how continuum calcs are done
...
for iw=0L,nw-1L do begin
  ...
  if nt eq 1 then begin			;(EM, not DEM
    ...
    tmp=zab*avdem*cfnc & tmp([0,nt-1])=0.5*tmp([0,nt-1])
    ;	the 0.5 weighting at the ends is due to trapezoidal integration
    flx(iw)=mdt*total(tmp)
