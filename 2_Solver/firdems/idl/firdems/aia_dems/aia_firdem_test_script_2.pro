; This script computes and plots the response of our fast DEM method to a set of log-normal
; distributions, with specified width and total em ('width' and 'emtot' below), at temperatures
; ranging from 10^5.5 to 10^7.0 Kelvin.
.compile ../estimate_quantile.pro
.compile ../dist_median.pro
.compile ../firdem.pro
.compile ../firdem_get_pixel.pro
.compile aia_firdem_wrapper.pro
.compile aia_firdem_test_2.pro
logtmin = 5.5
logtmax = 7.0
tmin = 10^logtmin
tmax = 10^logtmax
...
demarr = aia_firdem_test_2(tempsin(0), tempsout, dem_in, tr_struct=tr_struct, logtmin=logtmin-0.5, logtmax=logtmax+0.5, sigma_logn=width*0.5, nx=nx, ny=ny )
...
for i=0,nt-1 do begin &$
	demarr(i,*) = aia_firdem_test_2(tempsin(i),tempsout,dem_in, tr_struct=tr_struct, chi2s=chi2s, nx=nx, ny=ny, times=time, emtot=emtot, logtmin=logtmin-0.5, logtmax=logtmax+0.5, sigma_logn=width*0.5) &$
	times(i) = time &$
	chi2arr(i,*,*) = chi2s &$
	print,i, nt &$
endfor
...
device,file="plots/aia_gresp_width"+width_str+"_"+emtot_str+"EM.eps",/encapsulated
!p.font=0
...
plot_image,posdemarr, min=0, max=cmax, scale=[xscale,yscale], origin=[x0,y0], xtitle = xt, ytitle = yt, title='AIA DEM Gaussian Response', yrange=yrange,/nosquare
