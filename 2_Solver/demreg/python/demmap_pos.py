...
_probe_state = {"fired": False}  # dump per-pixel probes only for the first pixel (i=0)
def demmap_pos(dd,ed,rmatrix,logt,dlogt,glc,reg_tweak=1.0,max_iter=10,rgt_fact=1.5,dem_norm0=None):
    ...
    _probe_state["fired"] = False   # reset so probes capture pixel i=0 (matches IDL)
    na=dd.shape[0]
    nf=rmatrix.shape[1]
    nt=logt.shape[0]
    #set up some arrays
...
def dem_pix(dnin,ednin,rmatrix,logt,dlogt,glc,reg_tweak=1.0,max_iter=10,rgt_fact=1.5,dem_norm0=0):
    ...
    nt=logt.shape[0]
    nmu=42
    # PARITY FIX: IDL uses findgen(51)/(51-1.0) (i.e. /50, spanning exactly min..max, no
    # offset). Upstream port used /(52-1.0)=/51 and +1e-8, giving a different ltt grid and
    # hence a different elogt (T resolution). Match IDL:
...
#  and no element 0 or less.
        ...
        if not _probe_state["fired"]:
            chk_dump('02_svd', sva=sva, svb=svb)
            chk_dump('03_lamb', lamb=lamb, piter=piter, rgt=rgt)
        ...
        for kk in np.arange(nt):
            ...
            elogt[kk]=dlogt[kk]
            # PARITY FIX: IDL takes the FWHM span (max-min of ltt above half-max) floored
            # at dlogt, only when >1 point qualifies, and does NOT divide by 2. Upstream
            # port divided by 2, dropped the dlogt floor, and used >0 not >1.
            if (np.sum(hm_mask) > 1):
